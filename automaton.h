...
struct _edge {
    ...
    size_t to;
    char symb;
    std::tuple<size_t, size_t, char> to_tuple() const;
};
...
public:
    ...
    automaton(size_t number_of_vertices, size_t start, const Container& finish, const std::vector<char>& alphabet);
    ...
    std::vector<size_t> get_reachable_vertices();
    bool is_finishing(size_t vertex) const;
    void clear_zero_vertices();
